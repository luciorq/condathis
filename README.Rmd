...
```{r include=FALSE}
local_temp_dir <- fs::path_temp("tmp")
temp_home_dir <- fs::path(local_temp_dir, "home")
withr::local_envvar(
  .new = list(
    `HOME` = temp_home_dir,
    `XDG_DATA_HOME` = fs::path(temp_home_dir, "data_home"),
    `USERPROFILE` = temp_home_dir,
    `LOCALAPPDATA` = fs::path(temp_home_dir, "data_home"),
    `R_USER_DATA_DIR` = fs::path(temp_home_dir, "data_home")
  )
)
```
