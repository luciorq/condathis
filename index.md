...
Tired of [`system()`](https://rdrr.io/r/base/system.html) calls that
work on your machine but break everywhere else? `condathis` is here to
help! It lets you create self-contained environments for your
command-line tools, so your R code runs reliably for you, your
...
R’s [`system()`](https://rdrr.io/r/base/system.html) and
[`system2()`](https://rdrr.io/r/base/system2.html) are powerful, but
they depend on tools being installed on the host system. This creates a
few problems:
...
[condathis](https://github.com/luciorq/condathis) solves these issues by
creating isolated environments for each tool.
...
With [condathis](https://github.com/luciorq/condathis), you can lock in
a specific version:
...
The package [condathis](https://github.com/luciorq/condathis) relies on
...
- It is not supported the use of output redirections in commands,
  ...
  - Instead of redirects (e.g. “\>”), use the argument
    ...
    multiple calls to
    [`condathis::run()`](https://luciorq.github.io/condathis/reference/run.md),
    using `stdout` argument to control the output and `stdin` to control
    the input of each command. P.S. The current implementation only
    supports files as the “STDIN”.
