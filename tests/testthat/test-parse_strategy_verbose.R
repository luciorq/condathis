testthat::test_that("Parse verbose strategy", {
  ...
  testthat::expect_equal(parse_strategy_verbose(TRUE), list(cmd = FALSE, output = TRUE))
  ...
  testthat::expect_error(
    parse_strategy_verbose(strategy = c(TRUE, "silent")),
    class = "condathis_error_invalid_verbose"
  )

  testthat::expect_equal(parse_strategy_verbose(strategy = c("silent", "full")), list(cmd = FALSE, output = FALSE))

  testthat::expect_equal(parse_strategy_verbose(strategy = c("output", "full")), list(cmd = FALSE, output = TRUE))

  testthat::expect_equal(parse_strategy_verbose(strategy = c(TRUE, FALSE)), list(cmd = FALSE, output = TRUE))

  testthat::expect_equal(parse_strategy_verbose(strategy = c(TRUE, TRUE, FALSE)), list(cmd = FALSE, output = TRUE))
})
